...
describe('Book Details Component', () => {
  ...
  it('calls back passing updated book upon form submit', () => {
    ...
    const onBookChangeSpy = jest.fn((book: Book | BookProperties) => Promise.resolve(book));
    const onBookChangeAsyncTest = asyncTestFor(onBookChangeSpy);
    const wrapper = mount(<BookDetails book={currentBook} onBookChange={onBookChangeAsyncTest.getFn()}/>);
  ...
  return {
    getFn() {
      return (...args: any[]): Promise<any> => {
        return fnPromise = fn.apply(null, args).then((fnPromiseResult: any) => {
    ...
    afterFnHaveBeenCalled(): Promise<any> {
      return new Promise(resolve => notifyOnFnAlreadyCalled = resolve).then(() => fnPromise);
    }
  }
}
