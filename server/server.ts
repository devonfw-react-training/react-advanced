...
const books = function () {
  ...
  try {
    const booksAsJson = readFileSync(initialBookDataFilePath, 'utf-8').toString();
  ...
  return {
    ...
    update(book: Book): Book | undefined {
      const originalBook = _books.find(currentBook => currentBook.id === book.id);
      if (originalBook) {
...
app.get('/api/books/:id', (req, res) => {
    ...
    book ? res.json(book) : res.status(404).json({error: `Book with id: ${id} not found`});
  }
);
...
app.post('/api/books', (req, res) => {
    ...
    updatedBook ? res.json(updatedBook)
      : res.status(404).json({error: `Book with id: ${bookToUpdate.id} not found`});
  }
);
